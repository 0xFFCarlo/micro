...
{
  CParticleEmitter* components_emitter = (CParticleEmitter*)microECSComponentsGet(cid_particle_emitter);
  const unsigned int components_count = microECSComponentsCount(cid_particle_emitter);
  ...
  for (int i = 0; i < components_count; i++) {
    ...
    microParticleEmitterSetPosition(emitter->emitterId, position->x + emitter->offsetX, position->y + emitter->offsetY);
  }
